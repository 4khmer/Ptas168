...
interface BuildingModalProps {
  open: boolean
  onClose: () => void
  existing?: BuildingDto | null
}

export function BuildingModal({ open, onClose, existing }: BuildingModalProps) {
  const t = useT()
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setErrors({ submit: (e as Error).message || t('propModal.errBuildingSave') })
    } finally {
      setLoading(false)
    }
  }
...
interface FloorModalProps {
  open: boolean
  onClose: () => void
  existing?: FloorDto | null
  buildingId: string
}

export function FloorModal({ open, onClose, existing, buildingId }: FloorModalProps) {
  const t = useT()
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setErrors({ submit: (e as Error).message || t('propModal.errFloorSave') })
    } finally {
      setLoading(false)
    }
  }
...
interface RoomModalProps {
  open: boolean
  onClose: () => void
  existing?: RoomDto | null
  floorId: string
  buildingId: string
}

export function RoomModal({ open, onClose, existing, floorId, buildingId }: RoomModalProps) {
  const t = useT()
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setErrors({ submit: (e as Error).message || t('propModal.errRoomSave') })
    } finally {
      setLoading(false)
    }
  }
