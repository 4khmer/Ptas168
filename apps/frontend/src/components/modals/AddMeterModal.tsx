...
import type { RoomServiceUi, AddMeterReadingInput } from '../../store'
import type { GroupedMeterReading } from '@ptas/sdk'

interface AddMeterModalProps {
  open: boolean
  onClose: () => void
  onConfirm: (data: AddMeterReadingInput & { recorder: string }) => void | Promise<void>
  lastReading: GroupedMeterReading | null
  services?: RoomServiceUi[]
}
 ...
 */
export default function AddMeterModal({ open, onClose, onConfirm, lastReading, services = [] }: AddMeterModalProps) {
  const t = useT()
