...
import type { MeterReadingLatest } from '@ptas/sdk'
...
function defaultBillPeriod(contractStart: string | null | undefined, today: Date = new Date()) {
  if (!contractStart) {
...
export default function StartBillModal({ open, onClose, roomId, onSuccess }: StartBillModalProps) {
  const t = useT()
  const {
    rooms,
  ...
  const [latestReadings, setLatestReadings] = useState<MeterReadingLatest[]>([])
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setErrors({ submit: (e as Error).message || t('modal.startBill.errCreate') })
    } finally {
      setSubmitting(false)
    }
  }
