...
export default function AddTenantModal({ open, onClose, roomId, roomName, roomPrice = 0 }) {
  ...
  async function handleLookup() {
    ...
    try {
      ...
      setErrors({ phone: e.message || t('modal.addTenant.errLookup') })
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleFinalSubmit() {
    ...
    try {
      ...
      setErrors({ submit: e.message || t('modal.addTenant.errAdd') })
    } finally {
      setLoading(false)
    }
  }
        ...
        <div>
          ...
          <div className="bg-[#e8ebe6] rounded-xl p-4 mb-4 flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-[#e8ebe6] flex items-center justify-center text-[#0e0f0c] font-bold text-[18px]">
              {(foundTenant.fullName || foundTenant.name || '?')[0]}
            </div>
            <div>
              <div className="text-[15px] font-bold text-[#0e0f0c]">{foundTenant.fullName || foundTenant.name}</div>
