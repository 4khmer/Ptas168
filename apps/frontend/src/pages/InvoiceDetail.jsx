...
function HeroCard({ inv, total, rate, isPaid, isCancelled, isOverdue, fmtDate, t, paymentMethod, paidAt, dueDate, statusLabel }) {
...
// fixed-service rows.
function LineItem({ icon: Icon, tone = 'neutral', label, detail, amount }) {
  const tones = {
...
export default function InvoiceDetail() {
  ...
  async function handleShareTelegram() {
    ...
    setTgStatus(null)
    try {
      const r = await invoicesApi.shareToTelegram(id)
      ...
      setTgStatus({ variant: 'error', message: e.message || t('invoiceDetail.tgShare.failed') })
    } finally {
      setTgSharing(false)
  ...
  const billDays    = inv.billDays  || (Math.round((periodEnd - periodStart) / 86400000) + 1)
  ...
  const fmtDate = d => formatFullDate(d)
