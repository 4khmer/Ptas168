...
// ── Image uploader ────────────────────────────────────────────────────────────
function ImageUploader({ label, value, onChange, circular = false, uploadLabel = 'Upload' }) {
...
function QrPicker({ value, onChange, label, t }) {
  const inputRef = useRef(null)
  ...
  async function handleFile(e) {
    ...
    try {
      ...
      setError(err.message || t('invSetup.qrErrUnreadable'))
    } finally {
      setBusy(false)
    }
  }
...
function PreviewModal({ open, onClose, settings, exchangeRate, t }) {
  if (!open) return null
