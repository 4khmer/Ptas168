...
import type { TenantDocumentDto } from '@ptas/contracts'
...
export default function TenantDetail() {
  ...
  const tenantRooms = id ? getTenantRooms(id) : []
  ...
  const [previewDoc,   setPreviewDoc]   = useState<TenantDocumentDto | null>(null)
  ...
  async function saveName() {
    ...
    try { await updateTenant(id!, { name: nameVal.trim() }) } catch (e) { setSaveError((e as Error).message) }
    setEditingName(false)
  }

  async function savePhone() {
    if (!phoneVal.trim()) { setEditingPhone(false); return }
    try { await updateTenant(id!, { phone: phoneVal.trim() }) } catch (e) { setSaveError((e as Error).message) }
  ...
  async function handlePhotoFile(e: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = ev => updateTenant(id!, { photo: (ev.target?.result as string) ?? null })
  ...
  async function handleDocsUpload(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const newDocs: TenantDocumentDto[] = await Promise.all(files.map(async f => ({
      ...
      setSaveError((err as Error).message || t('tenantDetail.errUpload'))
    } finally {
  ...
  async function handleDocDelete(docId: string) {
    ...
    try {
      ...
      setSaveError((err as Error).message || t('tenantDetail.errRemove'))
    }
  }
  ...
  function isImage(d: TenantDocumentDto) {
  ...
  const recentInvoices = tenantRooms
    .flatMap(entry => entry ? getInvoicesByRoom(entry.room.id) : [])
      ...
      <div className="page-content scrollbar-hide p-4 space-y-4" style={{ paddingBottom: '24px' }}>
        ...
        {tenantRooms.length > 0 && (
          <div>
            ...
            <Card padding={false}>
              {tenantRooms.map((entry, i) => {
                if (!entry) return null
                const { room, building, floor, contract } = entry
                return (
                <div key={room.id}>
                  <div
...
function DocPreview({ doc, onClose, t, fmtSize }: DocPreviewProps) {
