...
import type { InvoiceStatusResponse } from '@ptas/contracts'
...
export default function Billing() {
  ...
  useEffect(() => {
    loadInvoicesPage({ q: search, status: activeTab === 'all' ? undefined : activeTab as InvoiceStatusResponse, from: dateFrom, to: dateTo })
  ...
  const fmtPeriod = (inv: InvoiceUiDto) => formatShortDate(inv.periodStart)
  const fmtDue    = (inv: InvoiceUiDto) => inv.dueDate ? formatFullDate(inv.dueDate) : ''
  const fmtDate   = (str: string) => formatFullDate(str)
          ...
          {STATUS_TABS.map(tab => (
            ...
            >
              ...
              <span className={`ml-1.5 text-[11px] ${activeTab === tab.key ? 'text-[#0e0f0c]' : 'text-[#868685]'}`}>
                {byStatus[tab.key as keyof typeof byStatus] ?? 0}
              </span>
            </button>
          ))}
        </div>
