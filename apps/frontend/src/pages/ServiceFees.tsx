...
const ICON_MAP: Record<string, LucideIcon> = { Droplets, Zap, ParkingSquare, Wifi, Brush, Shirt, Package, PawPrint, Box }
const ICON_OPTIONS = Object.keys(ICON_MAP)

interface ServiceIconProps { name: string; size?: number }
function ServiceIcon({ name, size = 18 }: ServiceIconProps) {
...
export default function ServiceFees() {
  ...
  const [editModal, setEditModal] = useState<{ open: boolean; service: ServiceFeeDto | null }>({ open: false, service: null })
  ...
  async function toggleDefault(svc: ServiceFeeDto) {
    try {
      await updateMasterService(svc.id, { isDefault: !svc.isDefault })
    } catch (e) {
      setDeleteError((e as Error).message || t('svcFees.errUpdateDefault'))
    }
  }

  function openEdit(svc: ServiceFeeDto) {
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      setEditError((e as Error).message || t('svcFees.errSave'))
    } finally {
  ...
  async function handleDelete(id: string) {
    try {
      ...
      setDeleteError((e as Error).message || t('svcFees.errDelete'))
    }
  }
  ...
  async function handleAddNew() {
    ...
    try {
      ...
      setNewErrors({ submit: (e as Error).message || t('svcFees.errCreate') })
    } finally {
      setNewLoading(false)
    }
  }
