...
import type { RoomDto, RoomAssetDto } from '@ptas/contracts'
import type { UpdateContractArgs, UpdateRoomArgs } from '@ptas/sdk'
import {
...
const ICON_MAP: Record<string, LucideIcon> = { Droplets, Zap, ParkingSquare, Wifi, Brush, Shirt, Package, PawPrint, Box, Home }

interface ServiceIconProps { name: string; size?: number }
function ServiceIcon({ name, size = 15 }: ServiceIconProps) {
...
export default function RoomDetail() {
  ...
  const info = roomId ? getRoomWithStatus(roomId) : null
  if (!info || !roomId) return <div className="app-shell"><div className="p-4 text-[#454745]">{t('roomDetail.notFound')}</div></div>

  const { room, contract, tenant, occupied } = info
  ...
  function startEdit(field: string, val: unknown) { setEditField(field); setEditVal(String(val ?? '')) }

  async function saveEdit() {
    if (!contract) return
    const update: UpdateContractArgs = {}
    ...
    try { await updateContract(contract.id, update) } catch (_) { /* swallow */ }
    setEditField(null)
  }
...
interface MeterModeSettingsProps {
  room: RoomDto
...
function MeterModeSettings({ room, updateRoom, t }: MeterModeSettingsProps) {
  const mode = room.meterReadingMode === 'auto' ? 'auto' : 'manual'
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')

  async function setMode(next: 'manual' | 'auto') {
    if (next === mode) return
    setSaving(true)
    setError('')
    try {
      ...
      setError((e as Error).message || t('svcFees.errSave'))
    } finally {
      setSaving(false)
    }
  }
...
function ModeOption({ active, onClick, disabled, icon, title, description }: ModeOptionProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
type EditingAsset = RoomAssetDto | 'new' | null
...
// edit modal.
function RoomAssets({ room, updateRoom, t }: RoomAssetsProps) {
  const assets = room.assets || []
  const [editing, setEditing] = useState<EditingAsset>(null) // null | 'new' | asset object
  const [error, setError] = useState('')

  async function persist(next: RoomAssetDto[]) {
    setError('')
    try {
  ...
  async function handleSave(asset: RoomAssetDto) {
    ...
    <>
      ...
      {editing && (
        <AssetEditor
          ...
          onSave={handleSave}
          onDelete={editing !== 'new' ? () => handleDelete((editing as RoomAssetDto).id) : null}
        />
      )}
    </>
  )
...
interface AssetEditorProps {
  t: TFunction
  initial: RoomAssetDto | null
  onClose: () => void
  onSave: (asset: RoomAssetDto) => Promise<void>
  onDelete: (() => Promise<void>) | null
}

function AssetEditor({ t, initial, onClose, onSave, onDelete }: AssetEditorProps) {
  ...
  async function pickPhoto(e: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = ev => setPhotoUrl((ev.target?.result as string) ?? null)
