...
function methodIcon(method: string | null | undefined) {
  if (method === 'Cash') return <Banknote size={14} />
  if (method === 'QR Transfer') return <QrCode size={14} />
  return null
}

function fmtMoney(n: number | null | undefined) { return `$${(n ?? 0).toFixed(2)}` }
function fmtBankMoney(p: BankPaymentDto) {
...
function fmtDate(s: string | null | undefined) {
  return s ? formatFullDate(s) : ''
}

function renderInvoiceRow(inv: InvoiceUiDto, t: TFunction, navigate: NavigateFunction, fmtDateFn: (s: string | null | undefined) => string) {
  return (
    <div
...
function renderBankRow(p: BankPaymentDto, t: TFunction, fmtDateFn: (s: string | null | undefined) => string) {
  return (
...
export default function Payments() {
  ...
  const matchesSearch = (inv: InvoiceUiDto, q: string) => {
    if (!q) return true
  ...
  type CombinedItem =
    | { kind: 'invoice'; date: string | null | undefined; data: InvoiceUiDto }
    | { kind: 'bank'; date: string | null | undefined; data: BankPaymentDto }
  const allItems = useMemo<CombinedItem[]>(() => {
    if (activeTab !== 'all') return []
    const invoiceItems: CombinedItem[] = filteredPaid.map(inv => ({ kind: 'invoice', date: inv.paidAt, data: inv }))
    // Filter bank payments by activeTab='all' uses the same date/search
    // already baked into filteredBank.
    const bankItems: CombinedItem[] = filteredBank.map(p => ({ kind: 'bank', date: p.paidAt, data: p }))
    return [...invoiceItems, ...bankItems].sort((a, b) => new Date(b.date || 0).getTime() - new Date(a.date || 0).getTime())
